...
/**
 * getFormAction - Étape 2.1.3
 * Récupère un formulaire de don par ID (pour la page d'édition).
 */
...
export async function getFormAction(
...
): Promise<{ success?: true; form?: DonationFormDraft; error?: string }> {
  ...
  try {
    ...
    const res = await fetch(`${apiUrl}/api/v1/donation-forms/${formId}`, { cache: 'no-store' });

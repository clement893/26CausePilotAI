...
export default function AdvancedCharts({
    ...
    return (
      <svg viewBox="0 0 100 100" className="w-full h-full">
        ...
        {radarData.map((point, index) => {
          ...
          return (
            <text key={index} x={x} y={y} textAnchor="middle" dominantBaseline="middle" className="text-xs fill-gray-300">
              {point.label}
            </text>
          );
        })}
  ...
  const renderDonutChart = () => {
    ...
    return (
      <svg viewBox="0 0 100 100" className="w-full h-full">
        ...
        {/* Center text */}
        <text x="50" y="50" textAnchor="middle" dominantBaseline="middle" className="text-sm font-semibold fill-white">
          {total}
        </text>
      </svg>
    );
  ...
  const renderGaugeChart = () => {
    ...
    return (
      <svg viewBox="0 0 100 100" className="w-full h-full">
        ...
        />
        {/* Value text */}
        <text x={centerX} y={centerY + 10} textAnchor="middle" dominantBaseline="middle" className="text-2xl font-bold fill-white">
          {value}
        </text>
        {/* Min/Max labels */}
        <text x={centerX - radius - 5} y={centerY + 5} textAnchor="end" className="text-xs fill-gray-400">
          {min}
        </text>
        <text x={centerX + radius + 5} y={centerY + 5} textAnchor="start" className="text-xs fill-gray-400">
          {max}
        </text>
      </svg>
    );
  ...
  return (
    <div className={clsx('glass-effect bg-[#13131A] rounded-lg border border-gray-800 p-6', className)}>
